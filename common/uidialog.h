...
/** 提示条颜色：信息 / 警告 / 错误 / 确认。只影响角标和文案语气，不改业务。 */
...
/** QSS objectName：uiBadgeInfo / Warn / Err / Ask，两端样式表分别上色。 */
...
/**
 * 大表单弹窗：头（标题+说明）/ 身（调用方往 body() 里加控件）/ 底（按钮）。
 * 电站新建/编辑、地图导航、注销确认都用它。只换皮，不改 Socket 字段。
 */
...
public:
    ...
    /** 中间内容区，往这里 addWidget 表单控件。 */
    QVBoxLayout *body() const { return body_; }
    /** 底栏按钮行。一般用 addOk / addCancel，不必直接操作。 */
    QHBoxLayout *footer() const { return footer_; }

    /** 次要按钮（描边「幽灵」样式），调用方自己接 clicked。 */
...
/**
 * 通用提示/确认框。cancelText 空则只有一个按钮。
 * @return 点了确定/主按钮为 true。
 */
...
/** 是/否确认。danger=true 主按钮变红（注销、代结算）。 */
