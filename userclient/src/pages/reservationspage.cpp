...
ReservationsPage::ReservationsPage(QWidget *parent) : QWidget(parent)
{
    ...
    auto *bar = new QFrame;
    bar->setObjectName(QStringLiteral("searchCard"));
    auto *row = new QVBoxLayout(bar);
    row->setContentsMargins(14, 12, 14, 12);
    row->setSpacing(8);
    auto *title = new QLabel(u8("有效预约"));
    title->setObjectName(QStringLiteral("h2"));
    auto *subtitle = new QLabel(u8("默认保留 15 分钟，到期自动失效"));
...
void ReservationsPage::render()
{
    ...
    if (reservations_.isEmpty()) {
        ...
        auto *findButton = new QPushButton(u8("去找桩"));
        findButton->setObjectName(QStringLiteral("primary"));
        connect(findButton, &QPushButton::clicked, this, &ReservationsPage::findStationsRequested);
        listLayout_->addWidget(findButton);
    ...
    for (const QJsonValue &value : reservations_) {
        ...
        row1->setSpacing(8);
        auto *viewButton = new QPushButton(u8("查看电桩"));
        ...
        auto *startButton = new QPushButton(u8("开始充电"));
        ...
        row1->addWidget(viewButton, 1);
        row1->addWidget(startButton, 1);
        auto *row2 = new QHBoxLayout;
        row2->setSpacing(8);
        auto *navigateButton = new QPushButton(u8("导航"));
        ...
        row2->addWidget(navigateButton, 1);
        row2->addWidget(cancelButton, 1);
