...
static bool dashListening()
{
    QTcpSocket sock;
    sock.connectToHost(QHostAddress::LocalHost, 5000);
    return sock.waitForConnected(250);
}

static void openDashBrowser()
{
    const QString url = QStringLiteral("http://127.0.0.1:5000");
    if (QProcess::startDetached(QStringLiteral("wslview"), {url}))
        return;
...
/** 启动只读 Flask 大屏并用浏览器打开。找不到脚本或端口没起来会弹窗。 */
void MainWindow::openDash()
{
    ...
    if (!dashListening()) {
        const QStringList scripts{
            ...
            QDir(appDir).absoluteFilePath(QStringLiteral("../../dashboard/app.py")),
            QDir::homePath() + QStringLiteral("/ChargeHub-Linux/dashboard/app.py"),
            QDir::homePath() + QStringLiteral("/ChargeHubGithub-Linux/dashboard/app.py"),
        ...
        if (py.isEmpty()) {
            uiError(this, u8("打开 Web 大屏"),
                    u8("找不到 dashboard/app.py。请先在工程目录执行 bash scripts/rebuild.sh"));
            return;
        }
        QProcess proc;
        ...
        if (!proc.startDetached()) {
            uiError(this, u8("打开 Web 大屏"), u8("拉不起 python3 app.py，请确认已安装 python3-flask"));
            return;
        }
        for (int i = 0; i < 16 && !dashListening(); ++i) {
            QThread::msleep(200);
            QCoreApplication::processEvents();
        }
        if (!dashListening()) {
            uiError(this, u8("打开 Web 大屏"),
                    u8("大屏端口 5000 没有起来。看 /tmp/chargehub_dash.log，或执行：sudo apt install python3-flask"));
            return;
        }
    }
    openDashBrowser();
